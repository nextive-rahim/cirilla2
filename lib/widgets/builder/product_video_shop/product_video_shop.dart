  ...
  _reloadProduct() {
    if (_productsStore!.products.isEmpty) {
      ...
      _productsStore!.getProducts(customQuery: {
        ...
        'page': 1,
        'exclude': [...exProducts.map((e) => (e.id ?? '').toString())].join(','),
        'per_page': _limit,
      });
    ...
    if (widget.widgetConfig != null && _appStore.getStoreByKey(key) == null) {
      ProductsStore store = ProductsStore(
        ...
        categories: cate,
          ...
          'exclude': [..._watchedVideoStored, ...exProducts.map((e) => (e.id ?? '').toString())].join(','),
          'page': 1,
          'per_page': _limit,
        }).then((value) {
          _reloadProduct();
  ...
  Future<void> _loadMore(List<Product> products) async {
    _productsStore!.getProducts(
      customQuery: {
        ...
        'exclude': [
          ..._watchedVideoStored,
          ...products.map((e) => e.id.toString()),
          ...exProducts.map((e) => e.id.toString())
        ].join(','),
        'page': 1,
        'per_page': _limit,
      },
